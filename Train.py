        ...
        if cfg.batch_weight:
            if target.sum() == 0:
                ...
                loss = F.binary_cross_entropy_with_logits(
                    output[index].reshape(-1), target, pos_weight=weight)
        else:
            loss = F.binary_cross_entropy_with_logits(
...
def run(args):
    ...
    dst_folder = os.path.abspath(os.path.join(args.save_path, 'classification'))
    rc, size = subprocess.getstatusoutput('du --max-depth=0 %s | cut -f1'% src_folder)
    folders_to_copy = ['bin', 'config', 'data', 'model', 'utils']
    #exit()
    for folder in folders_to_copy:
        src = os.path.join(src_folder, folder)
        dst = os.path.join(dst_folder, folder)
        # Check disk usage (optional)
        rc, size = subprocess.getstatusoutput('du --max-depth=0 %s | cut -f1' % src)
        if rc != 0:
            raise Exception('Copy folder error : {}'.format(rc))

        # Copy the folder
        rc, err_msg = subprocess.getstatusoutput('cp -R %s %s' % (src, dst))
        if rc != 0:
